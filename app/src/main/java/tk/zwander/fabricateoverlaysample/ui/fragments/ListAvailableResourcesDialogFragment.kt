...
class ListAvailableResourcesDialogFragment : DialogFragment() {
    ...
    override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
        val info = requireArguments().getParcelable<ApplicationInfo>("appInfo")!!
        ...
        val adapter = ResourcePickerAdapter(emptyList()) { item ->
            ...
            val defaultVal = when (item.type) {
                TypedValue.TYPE_INT_BOOLEAN -> if (item.values.firstOrNull() == "true") 1 else 0
                TypedValue.TYPE_INT_COLOR_ARGB8,
                TypedValue.TYPE_INT_DEC,
                TypedValue.TYPE_DIMENSION -> item.values.firstOrNull()?.toIntOrNull() ?: 0
            ...
            parentFragmentManager.setFragmentResult("list_resource_added", Bundle().apply {
                putParcelable(
                    "entry",
                    FabricatedOverlayEntry(item.resourceName, item.type, defaultVal)
                )
            })
            dismiss()
        }
